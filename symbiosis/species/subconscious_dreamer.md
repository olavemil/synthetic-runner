...
| `concerns.md` | Current concerns and anxieties (written by subconscious phase) |
...
```
heartbeat():
  thinking_context = load thinking.md + dreams.md + concerns.md
  ...
  concerns = llm_generate(system=subconscious.md, content=subconscious_context)
  write concerns.md

  # Phase 3: Dreaming
  dreaming_context = format_context([thinking.md, concerns.md])
...
on_message(events):
  ...
  # Phase 2: Worry (events + concerns + visions → approach)
  approach = llm_generate(system=worry.md, content=events + concerns, context=visions)
